...
Implementation of the basic structures inspired by the Monoidal Computer papers
(Pavlovic et al.), adapted for modeling AI systems and their resource properties. (This is a symbolic model for visualization, not a complete formalization.)
...
I = Object("I")  # monoidal unit (target of delete ⊤)  # noqa: E741
...
class Morphism:
    ...
    def __rshift__(self, other: Morphism) -> Morphism:
        ...
        if self.tgt != other.src:
            raise TypeError(f"Cannot compose {self} >> {other}: tgt/src mismatch ({self.tgt} != {other.src})")

        def composed_impl(x: Any) -> Any:
            return other.impl(self.impl(x))

        new_prog: str | None = None
    ...
    @classmethod
    def pure(cls, name: str, src: Object, tgt: Object, fn: Callable[[Any], Any]) -> Morphism:
