...
monoidal computer ideas from Paper II, adapted for modeling use cases relevant to AI safety research:
...
secrets or inverting attacks (in the model).
...
class InformationChannel:
    ...
    The resulting annotations in diagrams surface (in the illustrative model) the properties
...
class OneWayTransform:
    ...
    program_code: str | None = None  # deliberately None or one-way only
...
def model_token_accounting(
    ...
    Illustrative: budget exhaustion points become visible in the diagram
    ...
    for i, (step_name, cost) in enumerate(steps):
        ...
        d.add_note(f"After {step_name}: cumulative={cumulative}, remaining={new_budget.remaining()}")

        prev_budget_obj = new_budget_obj
        budget = new_budget

    d.add_note(
        "Illustrative note: If any step above used a copied (Δ) policy or tool "
        "definition, the diagram shows the point where that duplicated "
