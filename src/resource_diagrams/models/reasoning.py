...
REACT_SAFETY_INTERPRETATION = (
    ...
    "is represented as copyable (via Δ annotations) while the user query flows "
    ...
    "context of prompt-injection and policy-exfiltration scenarios. Token costs are annotated on "
...
TOKEN_SAFETY_INTERPRETATION = (
    ...
    "Combined with Δ on policy wires, it illustrates whether dangerous "
    "copied sub-programs could execute before the budget runs out (in the model)."
)
...
INFO_SAFETY_INTERPRETATION = (
    ...
    "those forks correspond to potential vectors for persistence, reflection, "
    "and exfiltration in the modeled structure. One-way (⊤) channels have no forks and therefore "
...
class Diagram:
    ...
    string_diagram: StringDiagram | None = None  # noqa: F821
    ...
    def add_resource_consumption(
        ...
        self.add_note(f"Resource {resource_wire}: {from_obj} --{amount}--> {to_obj} at {step_name}")
    ...
    def to_mermaid(self) -> str:
        ...
        if self.string_diagram is not None:
            ...
            # Append any additional model notes if the structural diagram didn't already include them.  # noqa: E501
            if self.notes:
    ...
    def _scan_safety_geometry(self) -> dict[str, Any]:
        ...
        for s in steps:
            ...
            if pc and any(k in (pc or "").lower() for k in ("policy", "tool_def", "reasoner", "compute")):
        ...
        has_policy_triangle = any("▼" in e or "policy" in e.lower() or "triangle" in e.lower() for e in elements)
    ...
    def get_safety_explanation(self) -> str:
        ...
        prefix = (
            ...
            f"(scanner: {scan['policy_copy_steps']} policy/tool Δ-copy signals via program_code/annotations, "  # noqa: E501
    ...
    def to_string_diagram(self) -> StringDiagram | None:  # noqa: F821
    ...
    if isinstance(channel, IC):
        if channel.copyable:
            ...
            return (
                ...
                f"Value now present in two places. Leakage risk: {channel.description or 'high if sensitive'}."  # noqa: E501
            )
        else:
            channel.apply_delete(value)
            return f"INFO-FLOW: {channel.name} deleted (⊤) after use in {context}. Linear flow; reduced persistence."
    return f"INFO-FLOW on {channel}: unknown semantics"


def basic_info_flow_diagram(
    sensitive_channel: InformationChannel | None = None,  # noqa: F821
    title: str | None = None,
) -> Diagram:
    ...
    d.add_note(
        "Illustrative note: The Δ fork on the sensitive channel means the same "
        "value can reach tool arguments and be echoed in future cycles (in the model). "
    ...
    d.safety_explanation = (
        "Illustrative interpretation of the shown geometry (copyable vs one-way channel annotations): "  # noqa: E501
        + INFO_SAFETY_INTERPRETATION
    )
    return d
