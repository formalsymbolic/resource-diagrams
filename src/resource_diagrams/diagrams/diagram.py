...
class Box:
    ...
    Strengthened invariant (construction-time): when `morph` is supplied,
    any explicitly passed `src`/`tgt` must match the morph's (or be omitted
    as None); mismatches are rejected in __post_init__ so inconsistent
    (morph vs declared arity) Box states are unrepresentable. The label
    is always derived from morph when present (callers may pass a placeholder
    such as "").
    """
    ...
    morph: Morphism | None = None
    program_code: str | None = None  # for special labeling inside

    def __post_init__(self) -> None:
        if self.morph is not None:
            if self.src is not None and self.src != self.morph.src:
                raise ValueError(
                    f"Box(morph=...) may not specify a conflicting src "
                    f"(got {self.src}, morph has {self.morph.src})"
                )
            if self.tgt is not None and self.tgt != self.morph.tgt:
                raise ValueError(
                    f"Box(morph=...) may not specify a conflicting tgt "
                    f"(got {self.tgt}, morph has {self.morph.tgt})"
                )
...
class Triangle:
    ...
    encoded_morph: Morphism | None = None
...
DiagramElement = Wire | Box | Triangle | Fork | Stem | Sequential | Tensor
            ...
            if first_tgt is not None and second_src is not None:
                if str(first_tgt) != str(second_src):
                    ...
                    object.__setattr__(
                        ...
                        {
                            **self.metadata,
                            "arity_warning": f"Possible mismatch: {first_tgt} -> {second_src}",
                        },
                    )
...
def wire(obj: Object, label: str | None = None) -> Wire:
...
def triangle(program: str, tgt: Object, encoded_morph: Morphism | None = None) -> Triangle:
