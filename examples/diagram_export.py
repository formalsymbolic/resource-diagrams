...
The generated files are faithful transcriptions / reconstructions of the
figures in the original PDFs and can be rendered directly in GitHub,
VS Code, Obsidian, etc.

Run:
    python examples/diagram_export.py

(Works from a fresh clone with no install or PYTHONPATH, thanks to
bootstrap guard below. The script (re)generates exactly the four
canonical paper figures (01-04) under examples/exports/ using the
diagrams layer. See README for provenance of the AI model diagrams
...
from resource_diagrams.diagrams import MermaidRenderer, StringDiagram, box, fork, seq, tensor, triangle, wire
...
def main() -> None:
    ...
    # Build the canonical paper figures using the renderer (exactly 01-04)
    ...
    print()
    print("All diagrams written (exactly the 01-04 paper figures). You can now:")
