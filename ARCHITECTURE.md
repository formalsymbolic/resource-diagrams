...
2. **Formal foundations, practical ergonomics**
   - The design remains true to the core ideas from the Monoidal Computer papers (in the sense of illustrative models) — specifically the Paper I foundation (data services as comonoids, universal and partial evaluators, diagrammatic fixed-point construction ideas) together with practical modeling idioms for AI safety research (ReAct, resources, information flow).
...
**Current implementation scope**: illustrative realization (symbolic model) of the Paper I core ideas (data services, universal and partial evaluators, diagrammatic fixed-point construction) together with practical AI safety modeling idioms (ReAct, resources, information flow).
...
├── diagrams/                # String diagram construction + high-quality MermaidRenderer (illustrative reconstructions of paper figures 01-04 + general trees)
│   ├── diagram.py
│   ├── mermaid_renderer.py
│   └── ...
...
Contains the implementation of the Paper I structures (as an executable symbolic model). This layer is relatively stable and well-documented with references back to the source material (Def 4.1, Lemma 6.2, Prop 6.1, §3 comonoids).
...
- Machine-checked proofs in a proof assistant are not a current (or near-term) objective.
...
**Current scope**: The library provides an illustrative implementation (symbolic model) of the Paper I core ideas together with practical safety modeling idioms. The design focuses on the foundational categorical structures from the source papers alongside usable modeling patterns for AI systems.
