"""
paper_laws.py — Helpers that assert structural correspondences and diagrammatic
patterns from the Monoidal Computer papers (Pavlovic et al.) within this
executable model.

These provide machine-checkable witnesses (in the sense of this symbolic
implementation) for the key geometric constructions. Each helper is scoped
explicitly to the model. Diagram asserts use structural tree walks over
DiagramElement nodes rather than text heuristics.
...
def assert_programs_copy_to_identical_pairs(program_codes: list[str], obj: Object = Object("Ξ")) -> None:
    """Key axiom enabling diagrammatic fixed-point construction idea.

    Paper I §6 (graphical argument of Lemma 6.2): "programs p are basic data, and thus
    ...
    for p in program_codes:
        ...
        assert (p1, p2) == (p, p), f"programs must copy to identical pairs (δ∘p = p⊗p). Got ({p1!r}, {p2!r})"
...
def assert_universal_evaluator_law(mc: MonoidalComputer, program: str, input_val: Any, expected: Any) -> None:
    ...
    assert result == expected, (
        f"evaluator law violated for ⌈{program}⌉ on {input_val}: u(p, x)={result} != expected f(x)={expected}"
    )
...
def assert_smn_specialize_law(
    ...
    This is the demo approximation of the categorical s-m-n / currying.
...
def assert_phi_self_application_law(mc: MonoidalComputer, p: str) -> None:
    ...
    This is the diagrammatic heart of the fixed-point construction idea (no external
...
def assert_fixed_point_construction_law(mc: MonoidalComputer, p_code: str) -> None:
    """Assert the fixed-point construction idea (Prop 6.1 + Lemma 6.2 style)
    holds inside the symbolic model.

    Paper I §6 Prop 6.1: Every computation has a fixed point.
    The construction (realized via current evaluators + diagrams/03_*.mmd)
    ...
    *registered* so that apply(fp_code, anything) yields the witness e, and
    the law {p}(e) == e holds (checked inside build_fixed_point and exposed
    via traces).

    This is the executable counterpart of the diagrammatic argument on p.26.

    (Per this implementation's symbolic model; note that build_fixed_point
    ...
    # In current impl: fp_meaning is the result of Φ(p) in the model
    ...
    assert p_on_e == fp_meaning, f"Prop 6.1 fixed point law {{ {p_code} }}(e) == e failed for e={fp_meaning!r}"


def assert_diagram_illustrates_paper_geometry(diagram: StringDiagram | str, figure_ref: str) -> None:
    """Verify that a constructed diagram or rendered mmd contains the
    expected structural elements corresponding to canonical figures in the
    source papers (structural witness within this model).

    When a StringDiagram is passed, uses *structural* tree walk over
    DiagramElement nodes (Triangle / Fork / Stem + their properties)
    rather than loose substring heuristics. 

    For rendered str we retain targeted text presence checks.

    Used for integration tests of the diagrams layer.

    (Per this implementation's model + renderer output.)
    """
    if isinstance(diagram, StringDiagram):
        ...
        assert has_program_triangle, (
            f"{figure_ref} must contain a Triangle node (program as data ▼ p) in its DiagramElement tree"
        )
        if "fixed" in figure_ref.lower() or "phi" in figure_ref.lower():
            has_copy_fork = _has_fork(root)
            assert has_copy_fork, (
                f"{figure_ref} must contain a Fork node (Δ copy) in its tree per Lemma 6.2 construction"
            )
            ...
            _has_triangle(root, program_substr="phi")
        ...
        if "fixed" in figure_ref.lower() or "phi" in figure_ref.lower():
            ...
            assert "Φ" in text or "phi" in text.lower(), "fixed point rendered diagram must reference Φ"
